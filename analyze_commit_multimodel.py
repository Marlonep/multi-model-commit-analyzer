...
def print_model_scores_table(model_scores: List[ModelScore]):
    ...
    # Header
    print(f"{'Model':<20} {'Provider':<12} {'Code Quality':<12} {'Dev Level':<10} {'Complexity':<11} {'Est. Hours':<10} {'Time (s)':<8}")
    print("-" * 120)
    
    # Data rows
    for score in model_scores:
        dev_level_str = f"{score.dev_level:.1f} ({'Junior' if score.dev_level <= 1.5 else 'Mid' if score.dev_level <= 2.5 else 'Senior'})"
        print(f"{score.model_name:<20} {score.provider:<12} {score.code_quality:<12.1f} {dev_level_str:<10} {score.complexity:<11.1f} {score.estimated_hours:<10.1f} {score.response_time:<8.2f}")

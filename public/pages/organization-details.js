...
function displayPeopleSection() {
    ...
    members.forEach(member => {
        ...
        memberAvatar.onclick = () => window.location.href = `/pages/user-details.html?user=${encodeURIComponent(member.user)}`;
    ...
    if (viewAllLink) {
        viewAllLink.href = `/pages/organization-members.html?org=${encodeURIComponent(organizationName)}`;
    }
}
    ...
    for (let i = 0; i < 5; i++) {
        if (i < sortedUsers.length) {
            ...
            const leaderboardItem = `
                <div class="leaderboard-item" onclick="window.location.href='/pages/user-details.html?user=${encodeURIComponent(username)}'">
